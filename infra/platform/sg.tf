...
# ── AgentCore Runtime ─────────────────────────────────────────────────────────
# Attached to the ENIs AgentCore creates in the private subnets.

resource "aws_security_group" "agentcore" {
  name        = "${var.name}-agentcore"
  description = "AgentCore Runtime ENIs: HTTPS egress to VPC endpoints for ECR and CloudWatch Logs"
  vpc_id      = aws_vpc.this.id
  ...
  tags = merge(var.tags, { Name = "${var.name}-agentcore-sg" })
}
